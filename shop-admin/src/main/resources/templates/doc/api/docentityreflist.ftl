...
<script type="text/javascript">
    function deleteGoods(){
        ...
        if (items==0) {
            ...
            if(confirm('您确定要删除吗?')){
                $("#form_list").attr("action",APP_BASE+"/doc/api/docentityref/del");
                $('#form_list').submit();
            }
        }
    }
...
<div class="page">
    ...
    <form method="post" name="formSearch" id="formSearch" action="">
        ...
        <table class="tb-type1 noborder search">
            <tbody>
            <tr>
                <td><a href="${base}/doc/api/docentity/list?docid=${docid}" id="ncsubmit" class="btn" title="新增">新增</a>
            </tr>
            </tbody>
        </table>
    </form>
    ...
    <form method='post' id="form_list" >
        ...
        <table class="table tb-type2">
            ...
            <tr class="hover edit">
                ...
                <td ><a href="${base}/doc/api/docentitypro/list?entityid=${data.entityid}">${data.name}</a></td>
                <td >${data.code}</td>
                <td >${data.description}</td>
                <td align="left">
                	<p>
                		<a href="${base}/doc/api/docentityref/del?ids=${data.id}" title="删除">删除</a>&nbsp;&nbsp;
                	</p>
                 </td>
            </tr>
