...
<style>
	...
	.button-one{float:left;width:40%;padding-left:25px;}
...
$(function(){
	$("#confirm_button").click(function(){
		...
		var buyerMessage = $("textarea[name='buyer_message']").val(); //换货原因
		var goodsNum = $("#returnGoodsNum").val(); //换货数量
		//判断填写换货数量为0
		if(goodsNum==0){
			layer.open({content:'请您至少填写一个需要换货的商品数量!', time: 1});
			return false;
		}
		if(buyerMessage==''){
			layer.open({content:'请填写换货原因!', time: 1});
			return false;
		} 
		//商品多图
		var goodsImageMore = "";
		...
		var fmUrl = '${base}/m/authc/buyer/shopBarter';
		$.ajax({
             type: "post",
             url: fmUrl,
			 ...
			 success:function(data) {
				if(data.success){
					...
					layer.open({content:'换货提交失败!', time: 1});
				}
			}
         }); 
	});
...
<body class="user_bg">
<div class="phone_hd">
	<a class="back" href="${base}/m/authc/buyer/orderList?orderState=10">
		<img src="/leimingtech-front/res/html5/images/back.png" width="25" height="25">
	</a>返回
		...
		<table class="trade_table" cellpadding="0" cellspacing="0">
	    	<tbody>
	        	<tr>
	                ...
	                <td>
	                	<ul>
			           		 <li class="big"><span class="fl">订单换货</span></li>
			        ...
			        <div class="dd dd_01">
			         ...
			         <div class="dd dd_01">
						...
						<label>
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							本次换货：&nbsp;
							<input type="text" id="returnGoodsNum" name="returnGoodsNum" value="${orderGoods.goodsNum}" onChange="change(this)"/>            
				  		</label>
				        </span>
				        <span class="size"> </span>
			         </div>
...
<#else>
	<div class="wrap-shadow">
		<div class="wrap-all ncu-order-view"> 
			<p style="font-size: 30px;">您的订单已申请换货</p>
		</div>
	</div>
</#if>
