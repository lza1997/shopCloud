...
<script type="text/javascript">
	$(function(){
		...
		$("#confirm_button").click(function(){
			...
			var fmUrl = '${base}/m/authc/buyer/refundReturnDelivery1';
			$.ajax({
	             type: "post",
	             url: fmUrl,
				 ...
				 success:function(data) {
					 if(data.success){
							location.href="${base}/m/authc/buyer/returnList";
					}else{
					                ...
					                <div class="goods_item" id="itemInfo">
					                    <div class="goods_img">
					                    	<a target="_blank" href="${base}/product/detail?id=${refundReturn.goodsId}">
					                    		<img src="${imgServer}${refundReturn.goodsImage}" onload="javascript:DrawImage(this,60,60);">
					                    	</a>
					                    </div>
					                    <div class="goods_name">
					                    	<a target="_blank" href="${base}/product/detail?id=${refundReturn.goodsId}"> ${refundReturn.goodsName}</a>
					                    </div>
					                </div>
					            </div>
	        				</div>
