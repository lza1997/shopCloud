    ...
    <div class="fixed-bar">
        <div class="item-title">
            ...
            <ul class="tab-base">
                ...
                <li><a href="JavaScript:void(0);" class="current"><span>修改模板</span></a></li>
            </ul>
        </div>
    </div>
					...
					<form method="post" id="tpl_form" name="tpl_form" action="javascript:void(0);">
						...
						<div class="postage-tpl-head">
							<ul>
								...
								<input type="text" class="text" autocomplete="off" value="${transport.title}" name="title" style="height: 18px;width: 123px; margin-top: 0px;">
...
$(function(){
	$('div[class="postage-tpl"]').each(function(){
		...
		var tag = 0;
		<#if transport.transportExtendList??>
			<#list transport.transportExtendList as tel>
			if ('${tel.type}' == tplType){
				...
				if ('${tel.areaId}' == ''){
					...
					cell = cell.replace(/TRANSTYPE/g,'${tel.type}');
