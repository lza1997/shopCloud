...
function big_cart_remove(ids){
			var type=arguments[1];
      			var method = function(){
					   jQuery.post("http://瑞华信通/remove_goods_cart.htm",{"ids":ids},function(data){
							if(type=="one_combin"){
								window.location.href="http://瑞华信通/wap/goods_cart0.htm";
							}else{
								  if(data.code=="100"){
...
function change_spec(obj){
	...
	var div =jQuery("#standard_"+obj_id);
		...
		jQuery.post("http://瑞华信通/goods_cart1_spec.htm",{
			"cart_id":obj_id
			},function(data){
...
function confirm_change_spec(obj){
	...
	jQuery.post("http://瑞华信通/goods_cart1_spec_save.htm",{
			"id":id,
			"gsp":gsp
			},function(data){
				if(data.code==100){
...
<script>
//更新购物车的小计价格以及总价
	function change_quantity(goods_price,cart_id,input){
	    var subtotal_span = $('#gc_totalprice' + cart_id);//积分小计
	    var amount_span = $('#total_price');//积分总计
	    //暂存为局部变量，否则如果用户输入过快有可能造成前后值不一致的问题
	    var count = input.value;//购买数量文本框
	    if(!isPositiveNum(count)){
	    	layer.open({content:'请填写正确的数字', time: 1});
	    	$(input).val(1);
	    	return false;
	    }
	    $.getJSON('${base}/points/updatePointsCart?shopPointsCartId=' + cart_id + '&pointsGoodsNum=' + count,'POST', function(result){
	        if(result.result=='true'){
	            //更新成功
	            $(input).attr('changed', count);
	            amount_span.html("总计:"+result.amount+"分");
	...
	function subMit(){
		...
		if(checkId == ""){
			layer.open({content:'请选择兑换商品', time: 1});
			return false;
		}else{
			checkId = checkId.substring(0, checkId.length-1);
			window.location.href = "${base}/points/cartOrder?cartIds="+checkId;
		}
	}
</script>
</head>
...
<div class="phone_hd">
	<a class="back" href="${base}/points/myPointsIndex"><img src="${base}/res/html5/images/back.png" width="25" height="25" /></a>购物车
...
<form  method="post"  name="cart_form" target="_self" id="cart_form" action="">
    	...
    	<#list pointsCartlistVo.list as pointsCart>
	     	<li>
	     		<div class="li_box">
	          	<div class="check">
	           	<div class="checkbox">
	            	<input type="checkbox" id="id_${pointsCart.id}" mark="" value="${pointsCart.id}" name="id"  />
	            	<label for="id_${pointsCart.id}" id="label_${cart.cartId}"></label>
	           	</div>
	           </div>
	              <div class="img">
	               <a href="${base}/points/pointsDetail?id=${pointsCart.pgoodsId}" target="_blank">
	               	<img src="${imgServer}${pointsCart.pgoodsImage}" alt="${pointsCart.pgoodsName}" width="60" height="60" />
	               </a>
	              </div>
	              <div class="infro">
	               <span class="name">
	               	<a href="${base}/points/pointsDetail?id=${pointsCart.pgoodsId}">${pointsCart.pgoodsName}</a>
	               </span>
	               <span class="num">
	               	<a class="minus" href="javascript:void(0);" id="count_down_${pointsCart.id}" gc_id="${pointsCart.id}" f_sale_type="0">－</a>
	               	<input  name="goods_count_${pointsCart.id}" type="text" id="goods_count_${pointsCart.id}" f_sale_type="0" value="${pointsCart.pgoodsChoosenum}" onkeyup="change_quantity('${pointsCart.pgoodsPoints}','${pointsCart.id}',this);" gc_id="${pointsCart.id}"  goods_count="1" />
	               	<a class="plus" href="javascript:void(0);" id="count_up_${pointsCart.id}" gc_id="${pointsCart.id}" f_sale_type="0">+</a>
	               </span>
	              </div>
	 				<div class="fr">
	  				...
	  				<span class="price" id="gc_price${pointsCart.id}">兑换积分：${pointsCart.pgoodsPoints}</span>
	  				<span id="gc_totalprice${pointsCart.id}">${pointsCart.pgoodsPoints*pointsCart.pgoodsChoosenum}</span>
	  				<span class="del">
	   				<a href="javascript:void(0);" onclick="drop_cart_item('${pointsCart.id}');">
			 ...
			 if(id!="" && id!=undefined){
			   var dis = jQuery("#combin_other").css("display");
				   if(dis=="none"){
					   ...
					   jQuery.post("http://瑞华信通/combin_carts_detail.htm",
								 {"id":id
									 },function(data){
										 if(data.code==100){
    ...
    <div class="car_pay">
        ...
        <div class="total">
	        <p class="price total_pri" id="total_price">总计:${pointsCartlistVo.goodsTotalNums}分</p>
