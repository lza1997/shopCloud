...
jQuery(document).ready(function(){
	...
	jQuery(".group_top li").click(function(){
		...
		window.location.href="${base}/m/goods/goodslist1?searchType=${searchType}&keyword=${keyword}&sortField="+ searchtype + "&sortOrder=" + order;
	}) 	  
});<!--end-->
	...
	<div id="wrapper">
		<div id="scroller">
			...
			<div class="group_list" id="thelist"> 
					...
					<#assign newProductlist = goodsListTag("{'storeid':'${storeId}','order':'0','tagDataType':'2','pagesize':'16','commend':'1'}")>
							  <#if newProductlist?exists && newProductlist?size gt 0>
							      <#list newProductlist as newGoods>
							      
							  	<a href="${base}/m/goods/goodsdetail?id=${newGoods.goodsId}">
							    <dl>
							      <dt><img width="90" height="90" src=<#if newGoods.goodsImage!''>"${imgServer}${newGoods.goodsImage}"<#else>"${base}/res/images/common/default_goods_image.gif_small.gif"</#if> title="${newGoods.goodsName}" alt="${newGoods.goodsName}"></dt>
							      <dd>
							        <h3><b>${newGoods.goodsName}</b></h3>
							        <span><strong>¥${newGoods.goodsStorePrice}</strong></span> <em>${newGoods.commentnum}人评价，${newGoods.salenum}人已购买</em> 
							      </dd>
							    </dl>
							    </a> 
			 				</#list>
