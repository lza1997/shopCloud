...
<div class="page">
    ...
    <div class="fixed-empty"></div>
     <form  method="post" enctype="multipart/form-data" id="jvForm">
      ...
      <form>
				<input type="hidden" name="root" value="${root }"/>
			 <input type="submit" value="创建文件" onclick="this.form.action='v_add';" class="create-template"/>
	</form>
    ...
    <form method="post" id='form_list' action="o_delete">
        ...
        <tbody>
        <tr>
        	<td><input value="${resource.name}" class="checkitem" type="checkbox" name="names" id="${resource.name}"></td>
        	<td>
        	<img src="${base}/res/images/tpl/file/${resource.ico}.gif"/></td>
        	<td>
        	<#if resource.directory>
		<a href="v_list?root=${resource.name}">${resource.filename}</a>
		<#else>
		<a href="${frontBase+resource.name}"<#if resource.image> onmouseover="showImage(this)" onmouseout="hideImage(this)"</#if> target="_blank">${resource.filename}</a>
		</#if>
        	</td>
            <td>${resource.size} KB</td>
            <td>${resource.lastModifiedDate?string("yyyy-MM-dd HH:mm:ss")}</td>
            <td class="txt-link">
            
                <a <#if !resource.directory>href="o_downFile?name=${resource.name?url}" <#else>href="o_export?root=${root?url}&name=${resource.name?url}" </#if> >下载</a> |
                 <a href="v_rename?root=${root?url}&name=${resource.name?url}">重命名</a> |
              <a <#if resource.editable>href="v_edit?root=${root?url}&name=${resource.name?url}" <#else>disabled="disabled"</#if>>修改</a> |
                <a href="o_delete_single?root=${root?url}&name=${resource.name?url}" onclick="if(!confirm('确定要删除吗')) {return false;}" >删除</a>
            ...
            <tr colspan="15" class="tfoot">
                ...
                <td colspan="16"><label for="checkallBottom">全选</label>
                    &nbsp;&nbsp;<a href="JavaScript:void(0);" class="btn"
                                   onclick="delresourceAll()"><span>删除</span></a>
            </tr>
            </tfoot>
        </table>
    </form>
