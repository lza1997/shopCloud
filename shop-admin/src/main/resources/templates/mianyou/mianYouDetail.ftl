...
<div class="page">
    <div class="fixed-bar">
        <div class="item-title">
            <h3 style="color:#0099CC">满免邮</h3>
            <ul class="tab-base">
            	<li><a href="${base}/mianYou/list"><span>活动列表</span></a></li>
            ...
            <tbody>
				...
				<td class="align-center" width="250">
					<#if mianYou.startTimeStr??>
	                	${mianYou.startTimeStr?string("yyyy-MM-dd hh:mm")}
                	</#if>
                </td>
                <td class="align-center" width="250">
					<#if mianYou.endTimeStr??>
	                	${mianYou.endTimeStr?string("yyyy-MM-dd hh:mm")}
                	</#if>
				</td>
				<!-- 送礼品这个还要分情况显示 -->
					...
					<#list mianyouRulesList as mianYouRule>
						<table class="align-left" width="440">
							...
							<td class="align-left" width="430">
								<font color="#3A87AD">单笔订单满</font> <font color="#FF3300" style="font-weight:bold;">${mianYouRule.price }</font>
								<font color="#3A87AD">元  ，立免邮费 </font>
							</td>
						</table>
					</#list>
				</td>
				<td class="align-center">
					...
					<#if mianYou.state == 1>未发布</#if>
					<#if mianYou.state == 2>正常</#if>
					<#if mianYou.state == 3>取消</#if>
					<#if mianYou.state == 4>失效</#if>
					<#if mianYou.state == 5>结束</#if>
				</td>
            </tbody>
        </table>
    </form>
