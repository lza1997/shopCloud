...
<script type="text/javascript">
    ...
    function deleteParam(){
        ...
        if (items==0) {
            ...
            if(confirm('您确定要删除吗?')){
                $("#form_list").attr("action",APP_BASE+"/doc/api/param/delDoc");
                $('#form_list').submit();
            }
        }
    }
...
<div class="page">
    ...
    <form method="post" name="formSearch" id="formSearch" action="${base}/doc/api/param/list">
    ...
    <form method='post' id="form_list" >
        ...
        <table class="table tb-type2">
            ...
            <tr class="hover edit">
                ...
                <td >${data.exampleValue}</td>
                <td >${data.defalutValue}</td>
                <td >${data.description}</td>
                <td align="left">
                	<p>
                		<a href="${base}/doc/api/param/findById?id=${data.id}&docid=${docid}" title="修改">修改</a>&nbsp;&nbsp;
                		<a href="${base}/doc/api/param/del?ids=${data.id}" title="删除">删除</a>&nbsp;&nbsp;
                	</p>
                 </td>
            </tr>
